...
export default function CategoryDetail() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // Format category untuk API (ganti spasi dengan %20)
        const formattedCategory = category.replace(/'/g, "").replace(/ /g, "%20");
        const response = await productService.getByCategory(formattedCategory);
        setProducts(response.data);
        setError(null);
      } catch (err) {
        setError("Gagal memuat produk dalam kategori ini");
